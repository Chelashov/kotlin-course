...
abstract class BaseClass(
    ...
    //Потому что мы обращаемся не к нему, а к незащищенному полю privateVal в дочернем классе
    private val privateVal: String,

    // 2. объясни, почему это поле недоступно в main()
    //Потому что поле Protected: Члены класса доступны внутри класса и в производных классах.
    ...
    //это невозможно, так как поле приватное
    fun getAll(): String {
        return mapOf(
    ...
    //потому что класс, который она возвращает задан как protected
    ...
    //потому что в дочернем классе эта функция переписана через override
    ...
    //потому что она возвращает класс, который задан как private
...
class ChildrenClass(
    ...
    // там доступно публичное поле publicVal из родительского класса
    publicVal: String
) : BaseClass(privateVal, protectedVal, publicVal) {
    // 11. объясни, почему в main() доступна функция getAll() хотя её здесь нет
    //потому что она не защищенная и доступна отовсюду
    ...
    //потому что эта функция приватная и не может быть переопределена
    private fun privatePrint() {
    ...
    fun writeProtectedField(name: String) {
        protectedField = name
    }
...
fun main() {
    val children = ChildrenClass(privateVal = "ку", protectedVal = "ку", publicVal = "ёпта")
    children.publicField = "Антонио Бандерас"
    children.writeProtectedField("Проктор")
